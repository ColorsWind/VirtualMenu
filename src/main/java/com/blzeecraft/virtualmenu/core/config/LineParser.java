...
public class LineParser {
	...
	public static boolean registerAction(@NonNull String name, BiFunction<LogNode, ResolvedLineConfig, LineConfigObject> supplier) {
	...
	public static <T extends LineConfigObject> T parseFull(LogNode node, String s, Class<T> clazz) throws InvalidLineFormatException, InvalidLineObjectException{
		int index = s.indexOf("{");
		if (index < 0) {
		...
		if (!clazz.isAssignableFrom(rlc.getClass())) {
			throw new ClassCastException("预期得到一个 " + clazz.getTypeName() + " 的实例却得到 " + rlc.getClass().getTypeName() + " 的实例, 这很有可能是一个插件的BUG.");
		}
		return (T) rlc;
