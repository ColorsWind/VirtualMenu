 ...
 * 接收 Packet 的方法没有在这里声明, 需要调用 {@link UserManager} 的方法进行处理. 
 * @author colors_wind
 * @see {@link UserManager#handleEvent(PacketMenuClickEvent)}
 * @see {@link UserManager#handleEvent(PacketMenuCloseEvent)}
 *
 */
	...
	default void sendServerPacketWrap(IUser<?> user, AbstractWindowPacket<?> packet) {
		VirtualMenu.getScheduler().runTaskGuaranteePrimaryThread(() -> {
			try {
				...
				PluginLogger.warning(UserManager.LOG_NODE, "尝试向玩家发送 Packet 时出现异常.");
			}
		});
	}
