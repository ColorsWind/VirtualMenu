...
public class ConfMenuFactory {
	...
	public static PacketMenu convert(LogNode node, StandardConf conf) {
		...
		builder.type(type);
		// refresh
		String refresh = conf.global.refresh.orElse(UpdatePeriod.NEVER.name());
		builder.refresh(UpdatePeriod.get(refresh).orElseGet(() -> {
			PluginLogger.warning(node, "找不到刷新间隔类型: " + refresh + " 可用的类型: " + UpdatePeriod.typesToString());
